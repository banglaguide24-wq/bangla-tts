...
# Hugging Face Inference API (ফ্রি)
API_URL = "https://api-inference.huggingface.co/models/mistralai/Mistral-7B-Instruct-v0.1"

# HTML টেমপ্লেট (ইউজার ইন্টারফেস)
UI_HTML = """
<!DOCTYPE html>
<html lang="bn">
<head>
...
<div class="card">
    <div class="header">
        ...
        <div class="subtitle">
            ...
            <span class="badge">🔍 SEO-ফ্রেন্ডলি</span>
        </div>
    </div>
    ...
    <div class="footer">
        ⚡ AI-সহায়তায় · সম্পূর্ণ অরিজিনাল · কপিরাইট-ফ্রি
    </div>
</div>
...
<script>
    ...
    generateBtn.addEventListener('click', async function() {
        ...
        if (!title) {
            setStatus('দয়া করে একটি টাইটেল লিখুন।', 'error');
            return;
        }
...
# ============================
# ব্লগ পোস্ট টেমপ্লেট (যেটা আপনি দিয়েছেন)
# ============================
BLOG_TEMPLATE = """
...
<div class="custom-blog-post">
  <style>
    ...
    .custom-blog-post .toc {{
      background: #f1f5f9;
      padding: 16px 20px;
      border-radius: 18px;
      margin: 20px 0;
    }}
    .custom-blog-post .toc ul {{
      list-style: none;
      padding-left: 0;
      margin-top: 10px;
    }}
    .custom-blog-post .toc li {{
      margin-bottom: 8px;
    }}
    .custom-blog-post .toc a {{
      text-decoration: none;
      color: #0f172a;
      font-weight: 500;
      border-bottom: 1px dotted transparent;
    }}
    .custom-blog-post .toc a:hover {{
      color: #1e3c72;
      border-bottom-color: #1e3c72;
    }}
    .custom-blog-post .blog-image {{
      width: 100%;
      height: auto;
      margin: 20px 0;
      border-radius: 18px;
    ...
    .custom-blog-post .info-box, .custom-blog-post .warning-box, .custom-blog-post .highlight-box {{
      padding: 16px;
      border-radius: 18px;
      margin: 20px 0;
    ...
    .custom-blog-post .two-column {{
      display: grid;
      grid-template-columns: repeat(auto-fit, minmax(180px, 1fr));
      gap: 16px;
      margin: 20px 0;
    }}
    .custom-blog-post .stat-box {{
      background: #f1f5f9;
      padding: 16px;
      border-radius: 16px;
      text-align: center;
    }}
    .custom-blog-post .stat-number {{
      font-size: 1.5rem;
      font-weight: 700;
      color: #1e3c72;
    }}
    .custom-blog-post .clickable-ai {{
      background: #e6f7ff;
    ...
    .custom-blog-post .clickable-ai:hover {{
      background: #1e3c72;
      color: white;
    }}
    .custom-blog-post .hidden-compare {{
      display: none;
      background: #f9f9ff;
      padding: 16px;
      border-radius: 18px;
    ...
    .custom-blog-post .faq-section {{
      margin: 24px 0;
    }}
    .custom-blog-post .faq-item {{
      background: #ffffff;
    ...
    .custom-blog-post .faq-item strong {{
      display: block;
      margin-bottom: 6px;
    }}
    .custom-blog-post .author-bio {{
      background: #fef9e6;
      border-radius: 18px;
      padding: 16px;
      margin: 28px 0 20px;
    ...
    .custom-blog-post .author-avatar {{
      width: 60px;
      height: 60px;
      background: #1e3c72;
      border-radius: 50%;
    ...
    @media (min-width: 640px) {{
      ...
      .custom-blog-post .author-avatar {{ margin: 0; }}
      .custom-blog-post .author-info {{ text-align: left; }}
    }}
    @media (max-width: 480px) {{
      .custom-blog-post h1 {{ font-size: 1.5rem; }}
      .custom-blog-post h2 {{ font-size: 1.3rem; }}
    }}
    .tip-card {{
      background: #f8fafc;
      border-radius: 20px;
  ...
  <div class="ai-summary">
    <strong>🤖 Google AI Overview:</strong> {ai_summary}
  </div>
  ...
  <!-- Clickable AI -->
  <div class="clickable-ai" id="aiContentBtn">
    🤖 এআই টিপস: {ai_btn_text}
  </div>
  <div id="aiContentResult" class="hidden-compare">
    {ai_tips_html}
  </div>
  ...
  <div class="highlight-box">
    <h3 style="margin-top:0;">📌 আপনার ব্লগের নাম-এর শেষ কথা</h3>
    <p>{conclusion}</p>
  </div>
  ...
  <footer>
    © ২০২৬ Your Blog - সমস্ত তথ্য সংগ্রহ ও রচনা | {author_name} | যোগাযোগ: info@yourblog.com
  </footer>
</div>
...
def parse_ai_output(text):
    # Try to extract JSON from the text (in case there's extra)
    try:
        # Find first { and last }
        start = text.find('{')
        end = text.rfind('}') + 1
        ...
        return {
            ...
            "tips": [{"title": f"টিপ {i+1}", "description": "বিস্তারিত description"} for i in range(10)],
...
"""
    ...
    html = BLOG_TEMPLATE.format(
        ...
        ai_btn_text=ai_btn_text,
        ai_tips_html=ai_tips_html,
        faq_items=faq_items,
        faq_title=faq_title,
...
def generate_post():
    ...
    try:
        ...
        response = requests.post(API_URL, headers=headers, json=payload, timeout=70)
        ...
        return "টাইমআউট: সার্ভার ব্যস্ত, আবার চেষ্টা করুন।", 504
